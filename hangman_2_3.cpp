//Thêm sự hiển thị khi end game nhưng cần làm gọn hàm renderWin và renderLose lại
#include <bits/stdc++.h>
#include <unistd.h>
using namespace std;
...
void renderWin (string secretWord);
void renderLose(string secretWord);
...
void renderWin(string secretWord)
{
    ...
    //Hỏi người chơi có chơi tiếp không và dùng đệ quy main nếu chơi tiếp.
    cout<< endl<< "Do you want to play again(Please type 'y'/ 'Y'/ 'YES'/ '1' to continue or type other character to stop) : ";
    char continueOrNot;
    cin>> continueOrNot;
    if( continueOrNot == 'y' || continueOrNot == 'Y' && continueOrNot == 'YES' || continueOrNot == '1' ){
        main();
...
void renderLose(string secretWord)
{
    ...
    cout<< "You lost. You are hanged. The correct word is "<< '"'<< ' '<< secretWord<< ' '<< '"' <<'\n' <<'\n';
    cout<<   " -------------   \n"
             " |           /   \n"
             " |          O   \n"
             " |         /|\\ \n"
